...
char pathout1[80] = {"/home/mtx/data/tide/outdata/old_test/c_test_data/0.000dens1.25.bin"};
char pathout2[80] = {"/home/mtx/data/tide/outdata/old_test/c_test_data/0.000deng1.25.bin"};
    ...
    for(k = 0; k < nn; k++)  {
        for(j = 0; j < nn; j++)  {
            for(i = 0; i < nn; i++)  {
                ...
                if(input <= 0) continue;//printf("error!input=%f\n", input);
                lnd = log(input); 
                fwrite(&lnd,   4, 1, OUT2);
            }
        }
