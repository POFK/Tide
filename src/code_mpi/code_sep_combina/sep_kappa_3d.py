...
if rank==0:
    ...
    gm1_x=gm1_x.astype(np.float64)
    gm2_x=gm2_x.astype(np.float64)
    gmx_x=gmx_x.astype(np.float64)
    gmy_x=gmy_x.astype(np.float64)
    gmz_x=gmz_x.astype(np.float64)
    ...
    k_mag[0,0,0]=1*Kf
    kappa_3dk=np.empty((N,N,N/2+1),dtype=np.complex128)
