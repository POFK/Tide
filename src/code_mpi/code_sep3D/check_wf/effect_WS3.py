...
W=np.loadtxt('/project/mtx/output/tides10/test/Wiener.txt')
...
#plt.semilogx(k,smooth(sigma=3.0),'g-',linewidth=3,label='smooth 1.25')
plt.semilogx(W[:,0],W[:,1],'r-',linewidth=3,label='only wiener')
plt.semilogx(W[:,0],W[:,1],'bo')
for i in np.linspace(0.80,1.30,(1.30-0.80)/0.05,endpoint=False):
    print i
    plt.semilogx(PH[:,0],W[:,1]*smooth_PH(sigma=i),'-',label='wiener & smooth_%3.2f'%i)
plt.ylim(10**-3,1)
plt.xlim(0.005,2.5)
